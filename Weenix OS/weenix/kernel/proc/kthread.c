...
kthread_clone(kthread_t *thr)
{
        ...
        //clone_thread->kt_proc = thr->kt_proc; 
        clone_thread->kt_retval = NULL;       
        clone_thread->kt_errno = 0;           
        clone_thread->kt_cancelled = 0;       
        clone_thread->kt_state = KT_RUN;      
        clone_thread->kt_wchan = NULL;        


        list_link_init(&clone_thread->kt_qlink);
        list_link_init(&clone_thread->kt_plink);
       
        //context_setup(&clone_thread->kt_ctx, NULL, 0, NULL, clone_thread->kt_kstack, DEFAULT_STACK_SIZE, thr->kt_proc->p_pagedir);
        KASSERT(KT_RUN == clone_thread->kt_state); 
        dbg(DBG_PRINT, "(GRADING3A 8.a)\n");
        return clone_thread;
