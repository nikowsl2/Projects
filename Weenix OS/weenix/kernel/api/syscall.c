...
sys_read(read_args_t *arg)
{
        ...
        //         dbg(DBG_PRINT, "(GRADING3A 3C? 2)\n");
        //         return -1;
        // }

        size_t total_bytes = kern_args.nbytes;
        size_t bytes_read = 0;
        while (bytes_read < total_bytes) {
                size_t bytes_to_read = MIN(PAGE_SIZE, total_bytes - bytes_read);
                int read_result;
                if ((read_result = do_read(kern_args.fd, new_page, bytes_to_read)) < 0) {                        
                ...
                copy_to_user((char *)kern_args.buf + bytes_read, new_page, read_result);
                // if ((err = copy_to_user((char *)kern_args.buf + bytes_read, new_page, read_result)) < 0) {
                //         page_free(new_page); 
                //         curthr->kt_errno = -err;
                //         dbg(DBG_PRINT, "(GRADING3A 3C? 4)\n");
                //         return -1;
                // }
...
sys_write(write_args_t *arg)
{
        ...
        while (bytes_written < total_bytes) {
                size_t bytes_to_write = MIN(PAGE_SIZE, total_bytes - bytes_written);
                copy_from_user(new_page, (char *)kern_args.buf + bytes_written, bytes_to_write);
                // if ((err = copy_from_user(new_page, (char *)kern_args.buf + bytes_written, bytes_to_write)) < 0) {
                //         page_free(new_page); 
                //         curthr->kt_errno = -err;
                //         dbg(DBG_PRINT, "(GRADING3A 3C? 3)\n");
                //         return -1;
                // }
                int write_result;
                if ((write_result = do_write(kern_args.fd, new_page, bytes_to_write)) < 0) {
                        page_free(new_page); 
                ...
                // if ((size_t)write_result < bytes_to_write) {
                //         dbg(DBG_PRINT, "(GRADING3A 3C? 5)\n");
                //         break;
                // }
                dbg(DBG_PRINT, "(GRADING3D 1)\n");
        }
...
sys_getdents(getdents_args_t *arg)
{
        ...
        while (dir_count < maxdir) {
                int curr_dirent_size = do_getdent(kern_args.fd, &dirent);
                if(curr_dirent_size < 0){
                        curthr->kt_errno = -curr_dirent_size;
                        dbg(DBG_PRINT, "(GRADING3D 1)\n");
                        return -1;
                }
                else if(curr_dirent_size != 0){
                        char *destination = (char *)kern_args.dirp + dir_count * sizeof(dirent_t);
                        copy_to_user(destination, &dirent, sizeof(dirent_t));
                        // if (err < 0) {
                        //         curthr->kt_errno = -err;
                        //         dbg(DBG_PRINT, "(GRADING3A 3C? 3)\n");
                        //         return -1;
                        // }
                        bytes_read += curr_dirent_size;
                        dir_count++;
                        dbg(DBG_PRINT, "(GRADING3D 1)\n");
                } 
                else {
