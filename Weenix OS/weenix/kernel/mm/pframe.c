...
pframe_get(struct mmobj *o, uint32_t pagenum, pframe_t **result)
{
        ...
        pframe_t *target = pframe_get_resident(o, pagenum);
        if (target != NULL) {
                if (pframe_is_busy(target)) {
                        while (pframe_is_busy(target)) {
                                sched_sleep_on(&(target->pf_waitq));
                                target = pframe_get_resident(o, pagenum);
        ...
        target = pframe_alloc(o, pagenum);
        int fill_result = pframe_fill(target);
