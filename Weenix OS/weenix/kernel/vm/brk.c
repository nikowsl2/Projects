...
do_brk(void *addr, void **ret)
{
        ...
        //         dbg(DBG_PRINT, "(GRADING3? brk3)\n");
        //         return -ENOMEM;
        // }

        vmmap_t *vmmap = curproc->p_vmmap;       
        uint32_t startvfn = ADDR_TO_PN((uint32_t)PAGE_ALIGN_UP((uint32_t)curproc->p_brk));
        uint32_t endvfn = ADDR_TO_PN((uint32_t)PAGE_ALIGN_UP((uint32_t)addr));
        

        if (endvfn < startvfn) {
                vmmap_remove(vmmap, endvfn, (startvfn - endvfn));
                dbg(DBG_PRINT, "(GRADING3D 1)\n");
        }

        else if(startvfn < endvfn){  
                if (!vmmap_is_range_empty(vmmap, startvfn, (endvfn - startvfn))) {
                        dbg(DBG_PRINT, "(GRADING3D 2)\n");
                        return -ENOMEM;
                }
                vmarea_t *brk_vma = vmmap_lookup(vmmap, startvfn - 1);
                brk_vma->vma_end = endvfn;    
                dbg(DBG_PRINT, "(GRADING3A)\n");
        }
