        ...
        int fd, off_t off, void **ret)
{
        ...
        int result = vmmap_map(curproc->p_vmmap, 
                             file ? file->f_vnode : NULL,
                             lopage, npages, prot, flags, off,
                             VMMAP_DIR_HILO, &new_vmarea);
        ...
        *ret = PN_TO_ADDR(new_vmarea->vma_start);
        ...
        if ((flags & MAP_PRIVATE) || (flags & MAP_ANON)) {
                ...
                (uintptr_t)PN_TO_ADDR(new_vmarea->vma_start),
                (uintptr_t)PN_TO_ADDR(new_vmarea->vma_start) + (uintptr_t)PAGE_ALIGN_UP(len));

                tlb_flush_range((uintptr_t)PN_TO_ADDR(new_vmarea->vma_start), npages);
                dbg(DBG_PRINT, "(GRADING3A)\n");
        }
...
do_munmap(void *addr, size_t len)
{
        ...
        int result =vmmap_remove(curproc->p_vmmap, lopage, npages);
        // if (result < 0) {
        //         dbg(DBG_PRINT, "(GRADING3? dommunmap4)\n");
        //         return result;
        // }
        ...
        pt_unmap_range(curproc->p_pagedir, 
                (uintptr_t)addr, 
                (uintptr_t)PN_TO_ADDR(ADDR_TO_PN((uintptr_t)addr) + npages));
        dbg(DBG_PRINT, "(GRADING3D 1)\n");
        return result;
