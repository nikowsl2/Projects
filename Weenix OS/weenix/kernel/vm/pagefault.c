...
handle_pagefault(uintptr_t vaddr, uint32_t cause)
{
        ...
        vmarea_t *vmarea = vmmap_lookup(curproc->p_vmmap, vfn);
        ...
        if (!((cause & FAULT_WRITE) || (cause & FAULT_EXEC)) && !(vmarea->vma_prot & PROT_READ)){
                dbg(DBG_PRINT, "(GRADING3D 2)\n");
                do_exit(EFAULT);
                panic("should not get here");
        }
        ...
        int err = pframe_lookup(vmarea->vma_obj, 
                              vfn - vmarea->vma_start + vmarea->vma_off,
                              forwrite, 
                              &error_pf);
        ...
        if (err < 0) {
                ...
                if (forwrite) {
                        ...
                        ptflags |= PT_WRITE;
                        pframe_pin(error_pf);
                        pframe_dirty(error_pf);
                        pframe_unpin(error_pf);
                        dbg(DBG_PRINT, "(GRADING3A)\n");
                }

                pt_map(curproc->p_pagedir, (uintptr_t)PAGE_ALIGN_DOWN(vaddr),
                        pt_virt_to_phys((uintptr_t)(error_pf->pf_addr)), 
                        pdflags, 
                        ptflags);
