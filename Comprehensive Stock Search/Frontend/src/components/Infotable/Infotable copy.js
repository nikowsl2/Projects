...
const Infotable = ({ companyData }) => {
    ...
    useEffect(() => {
        const checkBalance = async () => {
            try {
                const response = await fetch('/api/balance/search');
                if (!response.ok) {
    ...
    useEffect(() => {
        const checkIfInWatchlist = async () => {
            try {
                const response = await fetch(`/api/watchlist/search?ticker=${Ticker}`);
    ...
    const toggleWatchlistItem = async () => {
        try {
            ...
            if (isInWatchlist) {
                method = 'DELETE';
                endpoint = `/api/watchlist/delete`;
            }
            else {
                method = 'POST';
                endpoint = `/api/watchlist/insert`;
            }
    ...
    useEffect(() => {
        const fetchStockData = async () => {
            try {
                ...
                const response = await fetch(`/api/portfolio/search?ticker=${Ticker}`);
                if (!response.ok) {
    ...
    useEffect(() => {
        const fetchUpdatedData = async (symbol) => {
            try {
                const response = await fetch(`https://localhost:3001/watch-company?company_id=${symbol}`);
                if (!response.ok) {
    ...
    const BuyStock = async (event) => {
        ...
        try {
            ...
            const balanceResponse = await fetch('/api/balance/update', {
                method: 'POST',
    ...
    return (
        <div>
            <div className='messageContainer'>
                {WatchListMessage && <div className={`watchlist-message ${isInWatchlist ? 'in-watchlist' : ''}`}>{WatchListMessage}
