...
map<ScheduleKey, bool> RTHRoomList;
...
int sendNotificationToServerM(int udp_sock) {
    const char* message = "BOOT:ServerRTH";
    ...
    cout << "The Server RTH has informed the main server." << endl;
    return 0;
}

int sendAvailabilityResultToServerM(int udp_sock,const string& result) {
    string message = "Room request result from ServerRTH:\n" + result;
    sockaddr_in server_m_addr;
...
int runServerRTH() {
    ...
    while (getline(file, line)) {
        ...
        RTHRoomList[ScheduleKey(room, day, time)] = true;
    }

    file.close();
    cout << "The Server RTH is up and running using UDP on port " << UDP_PORT << ".\n";
...
string handleRoomReservation(const string& message) {
    ...
    for (const auto& entry : RTHRoomList) {
    ...
    auto it = RTHRoomList.find(key);
    if (it != RTHRoomList.end()) {
        if (it->second) {
            it->second = false;
...
string handleRoomAvailability(const string& message) {
    ...
    if (day.empty() && time.empty()) {
        ...
        ScheduleKey key(roomNumber, day, time);
        auto it = RTHRoomList.find(key);
        if (it != RTHRoomList.end() && it->second) {
